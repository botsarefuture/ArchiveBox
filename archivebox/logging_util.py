...
def log_archiving_finished(num_links: int):
    ...
    print(
        "    - {} links updated".format(
            _LAST_RUN_STATS.succeeded + _LAST_RUN_STATS.failed
        )
    )
    ...
    if Snapshot.objects.count() < 50:
        print()
        print(
            "    {lightred}Hint:{reset} To manage your archive in a Web UI, run:".format(
                **ANSI
            )
        )
...
def log_archive_method_finished(result: "ArchiveResult"):
    ...
    if result.status == "failed":
        if result.output.__class__.__name__ == "TimeoutExpired":
            ...
            hint_header = [
                "{lightyellow}Extractor timed out after {}s.{reset}".format(
                    duration, **ANSI
                ),
            ]
        else:
...
def log_removal_started(links: List["Link"], yes: bool, delete: bool):
    ...
    if delete:
        file_counts = [
            link.num_outputs for link in links if Path(link.link_dir).exists()
        ]
        print(
...
def printable_folders(
    folders: Dict[str, Optional["Link"]], with_headers: bool = False
) -> str:
    return "\n".join(
...
def printable_dependency_version(name: str, dependency: Dict) -> str:
    ...
    path = (
        str(dependency["path"]).replace(str(OUTPUT_DIR), ".")
        if dependency["path"]
        else ""
    )
    if path and " " in path:
