...
def merge_links(a: Link, b: Link) -> Link:
    ...
    timestamp = (
        a.timestamp
        if float(a.timestamp or 0) < float(b.timestamp or 0)
        else b.timestamp
    )
    ...
    for method in all_methods:
        ...
        history[method] = list(
            reversed(
                sorted(
                    (
                        ArchiveResult.from_json(pyjson.loads(result))
                        for result in deduped_jsons
                    ),
...
def validate_links(links: Iterable[Link]) -> List[Link]:
    ...
    try:
        ...
        links = sorted_links(
            links
        )  # deterministically sort the links based on timestamp, url
...
def links_after_timestamp(
    links: Iterable[Link], resume: Optional[float] = None
) -> Iterable[Link]:
    if not resume:
        yield from links
        return
...
def write_main_index(links: List[Link], out_dir: Path = OUTPUT_DIR) -> None:
    ...
    try:
        ...
        stderr(
            "    Run archivebox init to fix any inconsistencies from an ungraceful exit."
        )
        with timed_index_update(out_dir / SQL_INDEX_FILENAME):
            write_sql_main_index(links, out_dir=out_dir)
            os.chmod(
...
@enforce_types
def fix_duplicate_links_in_index(
    snapshots: QuerySet, links: Iterable[Link]
) -> Iterable[Link]:
    """
...
def dedupe_links(snapshots: QuerySet, new_links: List[Link]) -> List[Link]:
    ...
    new_links = [
        link for link in new_links if not snapshots.filter(url=link.url).exists()
    ]
