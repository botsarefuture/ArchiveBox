...
def run(
    ...
    **kwargs,
    ...
    try:
        ...
        with Popen(
            cmd, *args, start_new_session=start_new_session, **kwargs
        ) as process:
...
def atomic_write(
    ...
    encoding = (
        None if isinstance(contents, bytes) else "utf-8"
    )  # enforce utf-8 on all text writes

    # print('\n> Atomic Write:', mode, path, len(contents), f'overwrite={overwrite}')
    try:
        with lib_atomic_write(
            path, mode=mode, overwrite=overwrite, encoding=encoding
        ) as f:
...
def chmod_file(path: str, cwd: str = ".") -> None:
    ...
    if not root.exists():
        raise Exception(
            "Failed to chmod: {} does not exist (did the previous step fail?)".format(
                path
            )
        )
