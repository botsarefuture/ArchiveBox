...
def get_version(config):
    return json.loads(
        (Path(config["PACKAGE_DIR"]) / "package.json")
...
def get_commit_hash(config):
    try:
        return (
            list((config["PACKAGE_DIR"] / "../.git/refs/heads/").glob("*"))[0]
            .read_text()
            .strip()
        )
    except Exception:
...
DYNAMIC_CONFIG_SCHEMA: ConfigDefaultDict = {
    "TERM_WIDTH": {
        "default": lambda c: lambda: shutil.get_terminal_size((100, 10)).columns
    },
    ...
    "CONFIG_FILE": {
        "default": lambda c: (
            Path(c["CONFIG_FILE"]).resolve()
            if c["CONFIG_FILE"]
            else c["OUTPUT_DIR"] / CONFIG_FILENAME
        )
    },
    "COOKIES_FILE": {
        "default": lambda c: c["COOKIES_FILE"] and Path(c["COOKIES_FILE"]).resolve()
    },
    ...
    "CURL_VERSION": {
        "default": lambda c: bin_version(c["CURL_BINARY"]) if c["USE_CURL"] else None
    },
    ...
    "USE_WGET": {
        "default": lambda c: c["USE_WGET"] and (c["SAVE_WGET"] or c["SAVE_WARC"])
    },
    "WGET_VERSION": {
        "default": lambda c: bin_version(c["WGET_BINARY"]) if c["USE_WGET"] else None
    },
    ...
    "RIPGREP_VERSION": {
        "default": lambda c: (
            bin_version(c["RIPGREP_BINARY"]) if c["USE_RIPGREP"] else None
        )
    },
    "USE_SINGLEFILE": {
    ...
    "SINGLEFILE_VERSION": {
        "default": lambda c: (
            bin_version(c["SINGLEFILE_BINARY"]) if c["USE_SINGLEFILE"] else None
        )
    },
    ...
    "READABILITY_VERSION": {
        "default": lambda c: (
            bin_version(c["READABILITY_BINARY"]) if c["USE_READABILITY"] else None
        )
    },
    "USE_MERCURY": {"default": lambda c: c["USE_MERCURY"] and c["SAVE_MERCURY"]},
    "MERCURY_VERSION": {
        "default": lambda c: (
            "1.0.0" if shutil.which(str(bin_path(c["MERCURY_BINARY"]))) else None
        )
    },  # mercury is unversioned
    "USE_GIT": {"default": lambda c: c["USE_GIT"] and c["SAVE_GIT"]},
    "GIT_VERSION": {
        "default": lambda c: bin_version(c["GIT_BINARY"]) if c["USE_GIT"] else None
    },
    "SAVE_GIT": {"default": lambda c: c["USE_GIT"] and c["SAVE_GIT"]},
    "USE_YOUTUBEDL": {"default": lambda c: c["USE_YOUTUBEDL"] and c["SAVE_MEDIA"]},
    "YOUTUBEDL_VERSION": {
        "default": lambda c: (
            bin_version(c["YOUTUBEDL_BINARY"]) if c["USE_YOUTUBEDL"] else None
        )
    },
    ...
    "CHROME_VERSION": {
        "default": lambda c: (
            bin_version(c["CHROME_BINARY"]) if c["USE_CHROME"] else None
        )
    },
    ...
    "NODE_VERSION": {
        "default": lambda c: bin_version(c["NODE_BINARY"]) if c["USE_NODE"] else None
    },
...
def load_config_val(
    ...
    if type is None or val is None:
        ...
        if val.lower() in ("true", "false", "yes", "no", "1", "0"):
            raise ValueError(
                f"Invalid configuration option {key}={val} (expected a string)"
            )
        return val.strip()
        ...
        if not val.isdigit():
            raise ValueError(
                f"Invalid configuration option {key}={val} (expected an integer)"
            )
        return int(val)
...
def bin_version(binary: Optional[str]) -> Optional[str]:
    ...
    try:
        version_str = (
            run([abspath, "--version"], stdout=PIPE, env={"LANG": "C"})
            .stdout.strip()
            .decode()
        )
        if not version_str:
            version_str = (
                run([abspath, "--version"], stdout=PIPE).stdout.strip().decode()
            )
...
def bin_path(binary: Optional[str]) -> Optional[str]:
    ...
    return (
        shutil.which(str(Path(binary).expanduser()))
        or shutil.which(str(binary))
        or binary
    )
    ...
    return {
        ...
        "SQL_INDEX": {
            ...
            "is_mount": os.path.ismount(
                (config["OUTPUT_DIR"] / SQL_INDEX_FILENAME).resolve()
            ),
        },
    }
...
assert (
    TIMEZONE == "UTC"
), "The server timezone should always be set to UTC"  # we may allow this to change later
...
def check_dependencies(config: ConfigDict = CONFIG, show_help: bool = True) -> None:
    ...
    if config["TIMEOUT"] < 5:
        ...
        stderr(
            "    (Setting it to somewhere between 30 and 300 seconds is recommended)"
        )
        stderr()
        stderr(
            "    If you want to make ArchiveBox run faster, disable specific archive methods instead:"
    ...
    if config["USE_YOUTUBEDL"] and config["MEDIA_TIMEOUT"] < 20:
        ...
        stderr(
            "    Youtube-dl will fail to archive all media if set to less than ~20 seconds."
        )
        stderr("    (Setting it somewhere over 60 seconds is recommended)")
        stderr()
        stderr(
            "    If you want to disable media archiving entirely, set SAVE_MEDIA=False instead:"
        )
        stderr(
            "        https://github.com/ArchiveBox/ArchiveBox/wiki/Configuration#save_media"
        )
        stderr()
