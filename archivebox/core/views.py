    ...
    def get(self, request, path):
            ...
            try:
                try:
                    snapshot = Snapshot.objects.get(
                        Q(timestamp=slug) | Q(id__startswith=slug)
                    )
        ...
        try:
            try:
                ...
                try:
                    snapshot = Snapshot.objects.get(
                        Q(url="http://" + base_url(path))
                        | Q(url="https://" + base_url(path))
                    )
                except Snapshot.DoesNotExist:
    ...
    def get_queryset(self, **kwargs):
        ...
        if query and query.strip():
            ...
            try:
                ...
                print(
                    f"[!] Error while using search backend: {err.__class__.__name__} {err}"
                )
        return qs
...
class AddView(UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        return render(
            template_name=self.template_name, request=self.request, context=context
        )
