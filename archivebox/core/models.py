...
EXTRACTORS = [
    (extractor[0], extractor[0]) for extractor in get_default_archive_methods()
]
STATUS_CHOICES = [
    ("succeeded", "succeeded"),
    ("failed", "failed"),
...
class Snapshot(models.Model):
    ...
    def as_json(self, *args) -> dict:
        ...
        return {
            key: getattr(self, key) if key != "tags" else self.tags_str()
            for key in args
        }
    ...
    def archive_size(self):
        cache_key = (
            f"{str(self.id)[:12]}-{(self.updated or self.added).timestamp()}-size"
        )
    ...
    def headers(self) -> Optional[dict]:
        try:
            return json.loads(
                (Path(self.link_dir) / "headers.json")
...
class ArchiveResultManager(models.Manager):
    def indexable(self, sorted: bool = True):
        ...
        qs = self.get_queryset().filter(
            extractor__in=INDEXABLE_METHODS, status="succeeded"
        )
