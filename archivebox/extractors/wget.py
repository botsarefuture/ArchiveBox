...
def save_wget(
    ...
    cmd = [
        ...
        *(
            ["--restrict-file-names={}".format(RESTRICT_FILE_NAMES)]
            if RESTRICT_FILE_NAMES
            else []
        ),
...
def wget_output_path(link: Link) -> Optional[str]:
    ...
    search_dir = (
        Path(link.link_dir) / domain(link.url).replace(":", "+") / urldecode(full_path)
    )
    for _ in range(4):
            ...
            if search_dir.is_dir():
                ...
                for file_present in search_dir.iterdir():
                    if file_present == last_part_of_url:
                        return str(
                            (search_dir / file_present).relative_to(link.link_dir)
                        )
