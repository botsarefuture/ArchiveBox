...
def get_default_archive_methods():
    return [
        ...
        (
            ...
            should_save_title,
            save_title,
        ),  # keep title and readability below wget and singlefile, as it depends on them
...
def archive_link(
    ...
    if methods:
        ARCHIVE_METHODS = [method for method in ARCHIVE_METHODS if method[0] in methods]
    ...
    try:
        ...
        print(
            "    ! Failed to archive link: {}: {}".format(err.__class__.__name__, err)
        )
        raise
