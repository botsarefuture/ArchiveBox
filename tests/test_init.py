...
def test_add_link(tmp_path, process, disable_extractors_dict):
    ...
    add_process = subprocess.run(
        ["archivebox", "add", "http://127.0.0.1:8080/static/example.com.html"],
        capture_output=True,
...
def test_add_link_support_stdin(tmp_path, process, disable_extractors_dict):
    ...
    stdin_process.communicate(
        input="http://127.0.0.1:8080/static/example.com.html".encode()
    )
    archived_item_path = list(tmp_path.glob("archive/**/*"))[0]
...
def test_correct_permissions_output_folder(tmp_path, process):
    ...
    for file in index_files:
        ...
        assert oct(file_path.stat().st_mode)[-3:] in (
            OUTPUT_PERMISSIONS,
            DIR_PERMISSIONS,
        )


def test_correct_permissions_add_command_results(
    tmp_path, process, disable_extractors_dict
):
    os.chdir(tmp_path)
    add_process = subprocess.run(
        ["archivebox", "add", "http://127.0.0.1:8080/static/example.com.html"],
...
def test_collision_urls_different_timestamps(
    tmp_path, process, disable_extractors_dict
):
    os.chdir(tmp_path)
    subprocess.run(
    ...
    subprocess.run(
        ["archivebox", "add", "http://127.0.0.1:8080/static/iana.org.html"],
        capture_output=True,
        env=disable_extractors_dict,
    )
    ...
    first_archive = (
        tmp_path / "archive" / str(min([float(folder) for folder in archive_folders]))
    )
    ...
    init_process = subprocess.run(
        ["archivebox", "init"], capture_output=True, env=disable_extractors_dict
    )
    # 1 from duplicated url, 1 from corrupted index
    assert (
        "Skipped adding 2 invalid link data directories"
        in init_process.stdout.decode("utf-8")
    )
    assert init_process.returncode == 0


def test_collision_timestamps_different_urls(
    tmp_path, process, disable_extractors_dict
):
    os.chdir(tmp_path)
    subprocess.run(
...
def test_orphaned_folders(tmp_path, process, disable_extractors_dict):
    ...
    list_process = subprocess.run(
        ["archivebox", "list", "--json", "--with-headers"], capture_output=True
    )
    ...
    assert (
        "Added 1 orphaned links from existing JSON index"
        in init_process.stdout.decode("utf-8")
    )
    assert init_process.returncode == 0
