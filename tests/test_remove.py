...
def test_remove_regex(tmp_path, process, disable_extractors_dict):
    ...
    subprocess.run(
        ["archivebox", "remove", "--filter-type=regex", ".*", "--yes", "--delete"],
        capture_output=True,
    )
...
def test_remove_tag(tmp_path, process, disable_extractors_dict):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO core_tag (id, name, slug) VALUES (2, 'test-tag', 'test-tag')"
    )
    snapshot_ids = c.execute("SELECT id from core_snapshot")
    c.executemany(
        "INSERT INTO core_snapshot_tags (snapshot_id, tag_id) VALUES (?, 2)",
        list(snapshot_ids),
    )
    conn.commit()

    remove_process = subprocess.run(
        ["archivebox", "remove", "--filter-type=tag", "test-tag", "--yes", "--delete"],
        capture_output=True,
    )
...
def test_remove_before(tmp_path, process, disable_extractors_dict):
    ...
    conn = sqlite3.connect("index.sqlite3")
    c = conn.cursor()
    higherts, lowerts = timestamp = c.execute(
        "SELECT timestamp FROM core_snapshot ORDER BY timestamp DESC"
    ).fetchall()
    conn.commit()
    conn.close()
