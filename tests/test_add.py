...
def test_depth_flag_0_crawls_only_the_arg_page(
...
def test_depth_flag_1_crawls_the_page_AND_links(
    tmp_path, process, disable_extractors_dict
):
    arg_process = subprocess.run(
        [
...
def test_overwrite_flag_is_accepted(process, disable_extractors_dict):
    ...
    assert "unrecognized arguments: --overwrite" not in arg_process.stderr.decode(
        "utf-8"
    )
    assert "favicon" in arg_process.stdout.decode(
        "utf-8"
    ), "archive methods probably didnt run, did overwrite work?"
