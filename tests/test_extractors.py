...
def test_wget_broken_pipe(tmp_path, process, disable_extractors_dict):
    ...
    assert (
        "TypeError chmod_file(..., path: str) got unexpected NoneType argument path=None"
        not in add_process.stdout.decode("utf-8")
    )
...
def test_readability_works_with_singlefile(tmp_path, process, disable_extractors_dict):
    disable_extractors_dict.update(
        {"USE_READABILITY": "true", "USE_SINGLEFILE": "true"}
    )
    add_process = subprocess.run(
        ["archivebox", "add", "http://127.0.0.1:8080/static/example.com.html"],
        capture_output=True,
...
def test_use_node_false_disables_readability_and_singlefile(
...
def test_headers_ignored(tmp_path, process, disable_extractors_dict):
    add_process = subprocess.run(
        ["archivebox", "add", "http://127.0.0.1:8080/static/headers/example.com.html"],
        capture_output=True,
        env=disable_extractors_dict,
    )
    archived_item_path = list(tmp_path.glob("archive/**/*"))[0]
...
def test_headers_redirect_chain(tmp_path, process, disable_extractors_dict):
    ...
    add_process = subprocess.run(
        [
            "archivebox",
            "add",
            "http://127.0.0.1:8080/redirect/headers/example.com.html",
        ],
        capture_output=True,
        env=disable_extractors_dict,
    )
...
def test_headers_400_plus(tmp_path, process, disable_extractors_dict):
    disable_extractors_dict.update({"SAVE_HEADERS": "true"})
    add_process = subprocess.run(
        ["archivebox", "add", "http://127.0.0.1:8080/static/400/example.com.html"],
        capture_output=True,
        env=disable_extractors_dict,
    )
    archived_item_path = list(tmp_path.glob("archive/**/*"))[0]
